...
class OsteoporosisDenseNet121(nn.Module):
    def __init__(self, num_classes: int = 3, pretrained: bool = True):
        ...
        if pretrained:
            try:
                ...
                self.backbone = tv_models.densenet121(weights=DenseNet121_Weights.DEFAULT)
            except ImportError:
                # Fallback for older torchvision versions
                self.backbone = tv_models.densenet121(pretrained=True)
        else:
            self.backbone = tv_models.densenet121(weights=None)
            
        # Get input dimension for the classifier layer (usually 1024 for DenseNet-121)
    ...
    def forward(self, x: torch.Tensor, meta: torch.Tensor):
        # If image is grayscale (1 channel), repeat to 3 channels for DenseNet-121 compatibility
        if x.shape[1] == 1:
            x = x.repeat(1, 3, 1, 1)
            
        # Extract image features
        img_feats = self.backbone(x) # shape: (batch_size, 1024)
        ...
        combined_feats = torch.cat([img_feats, meta_feats], dim=1) # shape: (batch_size, 1040)
