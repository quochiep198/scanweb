...
1. **API Backend:** Tận dụng SQL Alchemy để gom nhóm (group by) các nhãn chẩn đoán của AI nhằm tính toán phân bố lâm sàng, tính toán tỷ lệ đồng nhất (Agreement Rate) dựa trên trạng thái review của bác sĩ, và truy vấn 5 ca chẩn đoán gần đây nhất sắp xếp theo thời gian giảm dần.
2. **UI Frontend:** Thiết kế UI Dashboard bằng cách áp dụng các lớp CSS đã được định nghĩa sẵn trong `dashboard.module.css`. Sử dụng các thẻ SVG tùy biến để xây dựng biểu đồ phân bố bệnh lý, vừa đảm bảo thẩm mỹ hiện đại của ứng dụng y khoa vừa giữ cho bundle frontend nhẹ và tối ưu hóa hiệu năng tải trang.
...
| --- | --- | ------------- | ------- |
| 1   | `backend/app/routers/dashboard.py` | Sửa | Cập nhật endpoint `/v1/dashboard/stats` để trả về thêm các trường thống kê mới và danh sách ca quét gần nhất. |
| 2   | `frontend/app/messages.ts` | Sửa | Bổ sung các chuỗi dịch tiếng Việt cho màn hình Dashboard mới. |
| 3   | `frontend/app/components/views/DashboardView.tsx` | Sửa | Viết lại mã UI hiển thị Welcome Banner, Account Card, biểu đồ SVG tỉ lệ chẩn đoán và bảng ca quét gần nhất. |
...
| 2   | Hợp đồng API  | Sửa | Mở rộng JSON response trả về từ `/v1/dashboard/stats`. |
...
| 6   | Thành phần FE | Sửa | Component DashboardView.tsx thay đổi hoàn toàn UI. |
...
| --- | ---- | ------------------ | ------------- |
| 1   | Cập nhật API stats trong Backend | `backend/app/routers/dashboard.py` | Chạy dev server backend, thực hiện truy cập endpoint `GET /v1/dashboard/stats` qua trình duyệt hoặc curl, xác nhận JSON trả về có cấu trúc chứa `distribution`, `agreement_rate`, `total_reviewed`, và `recent_measurements`. |
...
| 3   | Nâng cấp giao diện Dashboard Frontend | `frontend/app/components/views/DashboardView.tsx` | Chạy dev server frontend, kiểm tra màn hình Dashboard. Xác nhận hiển thị đúng: Lời chào bác sĩ, Thẻ tài khoản, Biểu đồ phân bố SVG, và Bảng 5 ca quét gần nhất. Kiểm tra xem các liên kết chuyển hướng nhanh đến tab chẩn đoán y khoa hoạt động đúng. |
